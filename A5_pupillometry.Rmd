...
data_pupil_summary_anticipation <- data_pupil%>%
  ungroup()%>%
  filter(probe == "anti")%>%
  group_by(condition,trial_type,sample_in_trial_t)%>%
  summarise(mean = mean(pupil_Avg_bc_scale, na.rm = T),
            lower = mean(pupil_Avg_bc_scale, na.rm = T) - qt(0.975, df = n() - 1) * sd(pupil_Avg_bc_scale, na.rm = T) / sqrt(n()),
            upper = mean(pupil_Avg_bc_scale, na.rm = T) + qt(0.975, df = n() - 1) * sd(pupil_Avg_bc_scale, na.rm = T) / sqrt(n()))
...
```{r}
t.test(ind_pupil_ant$mean_pupil[ind_pupil_ant$trial_type == "+$5"],
       ind_pupil_ant$mean_pupil[ind_pupil_ant$trial_type == "+$0"],
       paired = T)
effectsize::cohens_d(ind_pupil_ant$mean_pupil[ind_pupil_ant$trial_type == "+$5"],
       ind_pupil_ant$mean_pupil[ind_pupil_ant$trial_type == "+$0"],
       paired = T)
```
...
lme7b <- brm(PosA_scaled ~ mean_pupil_bc_ant * trial_type_binary + (1|Subject) + (1|condition),
              data_lm_anti)
summary(lme7b)
p_map(lme7b)
plot_model(lme7b, type = "pred", terms = c("mean_pupil_bc_ant","trial_type_binary"),
           show.data = T)+
  labs(title = "Anticipation Pos Arous")

lme8b <- brm(NegA_scaled ~ mean_pupil_bc_ant * trial_type_binary + (1|Subject) + (1|condition),
              data_lm_anti)
summary(lme8b)
p_map(lme8b)
plot_model(lme8b, type = "pred", terms = c("mean_pupil_bc_ant","trial_type_binary"),
           show.data = T)+
  labs(title = "Anticipation Neg Arous")
```
...
lmeC1 <- brm(mean_pupil_bc ~ reaction_outcome + (1|Subject) + (1|condition),
              data_pupil_C%>%
                filter(trial_type == "+$5"))
summary(lmeC1)
p_map(lmeC1)

lmeC2 <- brm(mean_pupil_bc ~ reaction_outcome + (1|Subject) + (1|condition),
              data_pupil_C%>%
                filter(trial_type == "-$5"))
summary(lmeC2)
p_map(lmeC2)
...
t <- lmer(arousal_scaled ~  mean_pupil_baseline_out + reaction_outcome + (1|Subject) + (1|condition),
              do)
p <-lmer(arousal_scaled ~  mean_pupil_bc_out * reaction_outcome+ (1|Subject) + (1|condition),
              do)
tp <- lmer(arousal_scaled ~  mean_pupil_nobc_out * reaction_outcome + (1|Subject) + (1|condition),
              do)
anova(t,p,tp)
r.squaredGLMM(t)
r.squaredGLMM(p)
...
m1 <- lmer(pupil_Avg_baseline ~ half + (1|Subject),data_pupil_tonic)
summary(m1)
